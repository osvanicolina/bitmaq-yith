            ...
            if ( ! isset( $_POST['rqa_name'] ) ) {
               return;
                ...
                if ( empty( $errors ) ) {
                    ...
                    global $wpdb;
                    $wpdb->insert('presupuestos', array(
                        'nombre' => $_POST['rqa_name'],
                        'email' => $_POST['rqa_email'],
                        'telefono' => $_POST['rqa_phone'], // ... and so on
                    ));
                    
                    $headers = "MIME-Version: 1.0\r\n"; 
                    $headers .= "Content-Type: text/html; charset=UTF-8"; 
                    $headers .= "From: Bitmaq <contacto@bitmaq.cl>\r\n"; 
                    
                     $mail = '<div style=" padding:5%; border: solid 3px navy; border-radius:10px;">
                    ...
                    $mail .= '<table cellspacing="0" cellpadding="6" style="width: 100%; border: 1px solid #eee;" border="1" bordercolor="#eee">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" style="text-align:left; border: 1px solid #eee;"> Link </th>
                                    </tr>
                                </thead>
                                <tbody>';
                        ...
                        foreach( $args['raq_content'] as $item ):
                            ...
                            $mail .= '<tr><td scope="col" style="text-align:left;">';
                            $mail .=$_product->get_title();
                            $mail .='</td>';
                            $mail .= '<td scope="col" style="text-align:left;">'.$item['quantity'].'</td>';
                            $mail .= '<td scope="col" style="text-align:left;"><a href="';
                            $mail .= get_permalink($item['product_id']);
                            $mail .='">';
                            $mail .='Ver</a>';
                            $mail .='</td></tr>';
                    ...
                    mail( $_POST['rqa_email'], 'Cotización', $mail, $headers);
                    
                    
                    
                    do_action( 'ywraq_process', $args ); 
                    do_action( 'send_raq_mail', $args );
                    wp_redirect( 'https://bitmaq.cl/cotizacion-finalizada/', 301 );
                    exit();
                }
            }
